...
2. In the **Project Setting** window, click the **Rosetta** tab.
    ...
    | |**Typeahead Intellisense**| Allows you to configure options whether to include **no plans to implement items** and **not yet implemented items** in the formula list intellisense. When **no plans to implement items** are selected, the formula names under *no plans to implement items* are displayed as ~~strikethrough~~. If **not yet implemented items** are selected, the font color of the formula names under **not yet implemented items** displays greyed-out. |
