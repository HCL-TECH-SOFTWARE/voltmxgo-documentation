...
- Volt MX Go components are now provided as plugins that can be installed on Volt Foundry and Volt Iris using the Volt MX Go Plugin Installer. This setup enables users to access the features of Volt MX Go while benefiting from regular updates to Volt Foundry and Volt Iris, including release versions, service packs, and fix packs.
